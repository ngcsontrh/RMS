...
namespace SRM.Server.Controllers
{
    ...
    public class ThanhQuaController : ControllerBase
    {
        ...
        public ThanhQuaController(IThanhQuaService ThanhQuaService)
        {
            _ThanhQuaService = ThanhQuaService;
        }

        [HttpPost("create")]
        public async Task<IResult> AddAsync([FromBody] ThanhQuaData data)
        {
            var result = await _ThanhQuaService.AddAsync(data);
            if (result)
            {
        ...
        [HttpPost("update")]
        public async Task<IResult> UpdateAsync([FromBody] ThanhQuaData data)
        {
            var result = await _ThanhQuaService.UpdateAsync(data);
            if (result)
            {
        ...
        {
            var result = await _ThanhQuaService.DeleteAsync(data.Id);
            if (result)
            {
                return Results.NoContent();
            }
        ...
        {
            var result = await _ThanhQuaService.GetListAsync();
            if (result.Item1)
            {
        ...
        {
            var result = await _ThanhQuaService.GetByIdAsync(id);
            if (result.Item1)
            {
