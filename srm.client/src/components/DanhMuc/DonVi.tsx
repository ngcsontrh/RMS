...
import { DonViData } from '../../models/data';
import { useState } from 'react';
import { DonViSearch } from '../../models/search';
import { useQuery } from '@tanstack/react-query';
import { getDonVis, editDonVi, createDonVi } from '../../services/DonViService';
import { Loading, Pagination, Error } from '../commons';

interface DataType extends DonViData {
    key: string;
    stt: number;
}
...
const columns: TableProps<DataType>['columns'] = [
    ...
    {
        title: <div style={{ textAlign: 'center' }}>Tên đơn vị</div>,
        key: 'ten',
        dataIndex: 'ten',
    },
];

const DonVi: React.FC = () => {
    const [form] = Form.useForm();
    const [messageApi, contextHolder] = message.useMessage();
    const [searchParams, setSearchParams] = useState<DonViSearch>({
        pageIndex: 1,
        pageSize: 10,
        ten: null,
    });
    const [editData, setEditData] = useState<DonViData | null>(null);
    ...
    return (
        <>
            {contextHolder}
            <Breadcrumb style={{ marginTop: '10px' }} items={[{ title: 'Danh mục' }, { title: 'Đơn vị' }]} />
