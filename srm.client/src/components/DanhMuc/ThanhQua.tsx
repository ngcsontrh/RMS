...
import { ThanhQuaSearch } from '../../models/search/ThanhQuaSearch';
...
const ThanhQua: React.FC = () => {
    ...
    const { data: thanhQuaDatas, isLoading, error, refetch } = useQuery({
        queryKey: ['thanhQuaData', searchParams],
        queryFn: () => getThanhQuas(searchParams),
    });

    const dataSource: DataType[] = thanhQuaDatas?.data?.map((item, index) => ({
        ...item,
        key: item.id!.toString(),
        stt: (searchParams.pageIndex! - 1) * searchParams.pageSize! + index,
    })) || [];

    const totalItems = thanhQuaDatas?.total ?? dataSource.length;
    ...
    const onFinish = async (values: any) => {
        try {
            if (editData?.id) {
                await editThanhQua(editData.id, { ...editData, ten: values.ten });
                messageApi.success('Cập nhật thành công');
            } else {
                await createThanhQua({ ten: values.ten });
            ...
            <Row gutter={15} style={{ marginTop: '10px' }}>
                ...
                <Col span={12}>
                    ...
                    <Form layout="vertical" form={form} onFinish={onFinish} style={{ marginTop: '10px' }} initialValues={{ ten: '' }}>
                        ...
                        >
                            <Input placeholder="Nhập tên thành quả" disabled={!isEditing} />
                        </Form.Item>
                        <Row justify="center">
                            <Form.Item>
                                {isEditing && (
