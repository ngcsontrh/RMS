...
export const createNoiDangBai = async (data: NoiDangBaoData): Promise<void> => {
    try {
...
export const editNoiDangBai = async (id: number, data: NoiDangBaoData): Promise<void> => {
    try {
...
export const getNoiDangBaiDropDownData = async (): Promise<NoiDangBaoData[]> => {
    try {
        const response = await api.get<NoiDangBaoData[]>(`${endpoint}/dropdown-data`);
        return response.data;
    } catch (e) {
        throw new Error("Xảy ra lỗi trong quá trình xử lý");
    }
